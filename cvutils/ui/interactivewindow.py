...
if __name__ == '__main__':
    ...
    window = InteractiveWindow("test_iw", autoexit=True)
    window.registerCallback(clickCallback, event=InteractiveWindow.EVENT_MOUSEDOWN)

    while True:
        if clicked_point is not None:
            cv2.circle(image, tuple(clicked_point.astype(int)), 10, (0, 0, 255), -1)
        window.showImg(image, time=1, disable_keys=False)
